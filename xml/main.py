...
def crea_modello_canonico(riga):
    ...
    # Pesi per l'IVP corretti (Età 5.1%)
    ivp_calc = ((eta_norm * 0.051) + (moca_inv * 0.414) + (updrs_norm * 0.318) + (vel_inv * 0.216)) * 100
    ivp_score = round(ivp_calc, 1)

    # CORREZIONE: Soglie definitive IVP dal Report Finale, modificate con limite a 50
    if ivp_score < 27.4:
        livello_rischio = "Danno assente/lieve (<27.4)"
    elif 27.4 <= ivp_score < 50.0:
        livello_rischio = "Danno moderato (27.4-50)"
    elif 50.0 <= ivp_score <= 75.0:
    ...
    if irp_score < 10:
        classe_irp = "Compenso/Stabile (<10%)"
    elif 10 <= irp_score < 20:
        classe_irp = "Allarme Precoce (10-20%)"
    elif 20 <= irp_score < 70:
        classe_irp = "Tipping Point (20-70%)"
    else:
        classe_irp = "Rottura Compenso (>70%)"
                ...
                <section>
                    ...
                    <text>
                        ...
                        <paragraph>
                            ...
                            La valutazione predittiva presentata in tabella è stata effettuata utilizzando un modello statistico di regressione logistica calibrato su un campione di 36 soggetti. Il modello presenta una curva ROC con un'area (AUC) di 0.972, garantendo per la soglia di cut-off clinica (IVP=27.4) una Sensibilità di 0.88 e una Specificità assoluta di 1.00 per la massima tutela dei soggetti sani.
                        </paragraph>
                    </text>{entries}
                </section>
            </component>
